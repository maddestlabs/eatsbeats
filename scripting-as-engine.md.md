...
Every clip in the DAW is driven by a script (Eatscript). The Piano Roll visual UI acts as a bi-directional parser over the underlying Lua script state.

## Data & Logic Layering (Approach 2)
To keep performance high and allow seamless bi-directional editing between UI and code:
1. **Declarative Data Table:** The visual Piano Roll updates a standard Eatscript array of note definitions (`notes = { {pitch=60, start=0, length=1, vel=100} }`).
...
2. WebAssembly/WASM Eatscript execution on Web targets vs. C/FFI on iOS/Android.
