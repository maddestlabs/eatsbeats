...
class EatProjectSerializer {
  /// Serializes a full [DawState] object into a formatted `.eats` script.
  static String serialize(DawState dawState, {String projectName = 'Untitled Song'}) {
    ...
    if (dawState.masterTrack.fxRack.isNotEmpty) {
      ...
      for (final fx in dawState.masterTrack.fxRack) {
        ...
        final scriptStr = fx.eatScriptCode != null && fx.eatScriptCode!.isNotEmpty ? ', eatScriptCode = ${_formatScriptString(fx.eatScriptCode!, '  ')}' : '';
        final paramsStr = fx.eatScriptParams.isNotEmpty ? ', eatScriptParams = ${_formatMap(fx.eatScriptParams)}' : '';
        buffer.writeln('    { id = "${_escapeString(fx.id)}", name = "${_escapeString(fx.name)}", type = "${fx.type.name}", enabled = ${fx.enabled}, mix = ${fx.mix.toStringAsFixed(2)}, params = ${_formatMap(fx.params)}$irStr$presetStr$scriptStr$paramsStr },');
      }
      buffer.writeln('  },');
      buffer.writeln();
    }
    ...
    if (dawState.songProcgenMeta != null && dawState.songProcgenMeta!.isNotEmpty) {
      buffer.writeln('  procgen = ${_formatDynamicValue(dawState.songProcgenMeta, "  ")},');
      buffer.writeln();
    }
    buffer.writeln('}');
  ...
  /// Serializes a list of [Note] objects into a self-contained `.eats` snippet
  /// suitable for clipboard export and external Eatscript scripting.
  ...
  static void _serializeTrack(StringBuffer buffer, TrackChannel track, {required String indent}) {
    ...
    // Eatscript plugin script & params
    if (track.eatScriptCode.isNotEmpty) {
      buffer.writeln('${childIndent}eatScriptCode = ${_formatScriptString(track.eatScriptCode, childIndent)},');
    }
    if (track.eatScriptParams.isNotEmpty) {
      buffer.writeln('${childIndent}eatScriptParams = {');
      track.eatScriptParams.forEach((key, val) {
    ...
    if (track.fxRack.isNotEmpty) {
      ...
      for (final fx in track.fxRack) {
        final irStr = fx.irSampleName != null ? ', irSampleName = "${_escapeString(fx.irSampleName!)}"' : '';
        final presetStr = fx.presetId != null ? ', presetId = "${_escapeString(fx.presetId!)}"' : '';
        final scriptStr = fx.eatScriptCode != null && fx.eatScriptCode!.isNotEmpty ? ', eatScriptCode = ${_formatScriptString(fx.eatScriptCode!, childIndent)}' : '';
        final paramsStr = fx.eatScriptParams.isNotEmpty ? ', eatScriptParams = ${_formatMap(fx.eatScriptParams)}' : '';
        buffer.writeln('$childIndent  { id = "${_escapeString(fx.id)}", name = "${_escapeString(fx.name)}", type = "${fx.type.name}", enabled = ${fx.enabled}, mix = ${fx.mix.toStringAsFixed(2)}, params = ${_formatMap(fx.params)}$irStr$presetStr$scriptStr$paramsStr },');
      }
      buffer.writeln('${childIndent}},');
    }
    ...
    if (track.midiFXRack.isNotEmpty) {
      ...
      for (final mfx in track.midiFXRack) {
        buffer.writeln('$childIndent  { id = "${_escapeString(mfx.id)}", name = "${_escapeString(mfx.name)}", enabled = ${mfx.enabled}, eatScriptCode = ${_formatScriptString(mfx.eatScriptCode, childIndent)}, eatScriptParams = ${_formatMap(mfx.eatScriptParams)} },');
      }
      buffer.writeln('${childIndent}},');
    }
  ...
  static void _serializeClip(StringBuffer buffer, TrackClip clip, {required String indent}) {
    ...
    if (clip.eatScriptCode.isNotEmpty) {
      buffer.writeln('${childIndent}eatScriptCode = ${_formatScriptString(clip.eatScriptCode, childIndent)},');
    }
    if (clip.eatScriptParams.isNotEmpty) {
      buffer.writeln('${childIndent}eatScriptParams = ${_formatMap(clip.eatScriptParams)},');
    }
  ...
  static String _formatScriptString(String str, String indent) {
    if (!str.contains('\n') && !str.contains('"')) {
  ...
  static String _formatDynamicValue(dynamic value, String indent) {
    ...
    if (value is List) {
      ...
      for (final item in value) {
        buffer.writeln('$childIndent${_formatDynamicValue(item, childIndent)},');
      }
      buffer.write('$indent}');
      return buffer.toString();
    }
    if (value is Map) {
      ...
      for (final entry in value.entries) {
        ...
        buffer.writeln('$childIndent$keyFormatted = ${_formatDynamicValue(entry.value, childIndent)},');
      }
      buffer.write('$indent}');
      return buffer.toString();
    }
