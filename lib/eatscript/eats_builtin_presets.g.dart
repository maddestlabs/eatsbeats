...
# --- Modular Insert Audio FX Graph Topology ---
def graph():
    audio_in = eat.node.input()
    driven = eat.node.saturate(audio_in, drive="Drive")
    crushed = eat.node.bitcrush(driven, bits="Bits", downsample="Downsample", mix="Mix")
    return crushed
...
''',
    ...
    EatScriptDef(
      ...
      category: EatScriptCategory.audioFx,
      description: 'Studio dynamic range compressor with adjustable threshold, ratio, soft knee, attack, release, and auto-makeup gain.',
      tags: ['compressor', 'dynamics', 'vca', 'sidechain', 'mixing', 'mastering', 'studio'],
      code: r'''
# @id: dynamics_compressor
# @name: Dynamics Compressor
# @category: audioFx
...
def init():
    ...
    return {
        ...
        "Knee": eat.param("Knee", 0.0, 24.0, 12.0, unit="dB"),
        ...
        "panel": {
            ...
            "layout": [
                ...
                },
                {
                    "type": "row",
                    "children": [
                        {"type": "knob", "param": "Knee", "label": "KNEE", "unit": "dB"},
                        {"type": "knob", "param": "Makeup", "label": "MAKEUP", "unit": "dB"},
...
# --- Modular Synthesis Graph Topology ---
def graph():
    in_sig = eat.node.input()
    comp = eat.node.compressor(in_sig, threshold="Threshold", ratio="Ratio", attack="Attack", release="Release", makeup="Makeup", mix="Mix")
    return comp
...
def graph():
    in_sig = eat.node.input()
    vcf = eat.node.moog(in_sig=in_sig, cutoff="Cutoff", reso="Resonance")
    return vcf
...
''',
    ...
    EatScriptDef(
      ...
      tags: ['limiter', 'brickwall', 'mastering', 'output', 'protection', 'studio'],
      code: r'''
# @id: master_limiter
# @name: Master Limiter
# @category: audioFx
...
# --- Parameter Definitions ---
def init():
    eat.use_engine("limiter")
    return {
        "Threshold": eat.param("Threshold", -24.0, 0.0, -1.0, unit="dB"),
        "Release": eat.param("Release", 0.01, 1.0, 0.05, unit="s"),
...
def graph():
    in_sig = eat.node.input()
    lim = eat.node.limiter(in_sig, ceiling="Ceiling", release="Release")
    return lim
...
''',
    ...
    EatScriptDef(
      id: 'multimode_filter',
      name: 'Multi-Mode Studio Filter',
      category: EatScriptCategory.audioFx,
      description: '8-mode analog modeled studio filter featuring Direct Form II Transposed topology with lowpass, highpass, bandpass, notch, and peaking modes.',
      tags: ['filter', 'lowpass', 'highpass', 'bandpass', 'notch', 'studio', 'vcf'],
      code: r'''
# @id: multimode_filter
# @name: Multi-Mode Studio Filter
# @category: audioFx
# @engine: multimode_filter
# @tags: filter, lowpass, highpass, bandpass, notch, studio, vcf
# @description: 8-mode analog modeled studio filter featuring Direct Form II Transposed topology with lowpass, highpass, bandpass, notch, and peaking modes.

# --- Parameter Definitions ---
def init():
    eat.use_engine("multimode_filter")
    return {
        "Cutoff": eat.param("Cutoff", 20.0, 20000.0, 2000.0, unit="Hz"),
        "Resonance": eat.param("Resonance", 0.1, 15.0, 1.2, unit="Q"),
        "Drive": eat.param("Drive", 0.0, 1.0, 0.15, unit=""),
        "Gain": eat.param("Gain", -24.0, 24.0, 0.0, unit="dB"),
        "Mode": eat.param("Mode", 0.0, 7.0, 0.0, step=1.0),
    }
...
def gui():
    return {
        "panel": {
            "title": "MULTI-MODE FILTER",
            "subtitle": "Direct Form II Transposed Studio VCF",
            "background": "carbon",
            "accent": "#FF007A",
            "layout": [
                {
                    "type": "row",
                    "children": [
                        {"type": "knob", "param": "Cutoff", "label": "CUTOFF", "unit": "Hz", "size": 64},
                        {"type": "knob", "param": "Resonance", "label": "RESONANCE", "unit": "Q"},
                        {"type": "knob", "param": "Drive", "label": "DRIVE"},
                        {"type": "knob", "param": "Gain", "label": "GAIN", "unit": "dB"},
                        {"type": "knob", "param": "Mode", "label": "MODE"}
                    ]
                }
            ]
        }
...
def graph():
    audio_in = eat.node.input()
    driven = eat.node.saturate(audio_in, drive="Drive")
    filtered = eat.node.svf(driven, cutoff="Cutoff", reso="Resonance")
    return filtered
...
''',
    ...
    EatScriptDef(
      id: 'parametric_eq',
      name: 'Parametric EQ 5-Band',
      category: EatScriptCategory.audioFx,
      description: '5-band studio parametric equalizer with highpass, low shelf, dual parametric peaking bells, and high shelf filters.',
      tags: ['eq', 'equalizer', 'mastering', 'mixing', 'filter', 'studio'],
      code: r'''
# @id: parametric_eq
# @name: Parametric EQ 5-Band
# @category: audioFx
...
# --- Parameter Definitions ---
def init():
    eat.use_engine("parametric_eq")
    return {
        "HPFFreq": eat.param("HPFFreq", 10.0, 500.0, 30.0, unit="Hz"),
        "LowFreq": eat.param("LowFreq", 30.0, 800.0, 100.0, unit="Hz"),
        "LowGain": eat.param("LowGain", -18.0, 18.0, 0.0, unit="dB"),
        "Mid1Freq": eat.param("Mid1Freq", 100.0, 4000.0, 650.0, unit="Hz"),
        "Mid1Gain": eat.param("Mid1Gain", -18.0, 18.0, 0.0, unit="dB"),
        "Mid1Q": eat.param("Mid1Q", 0.1, 10.0, 1.0, unit="Q"),
        "Mid2Freq": eat.param("Mid2Freq", 500.0, 12000.0, 2800.0, unit="Hz"),
        "Mid2Gain": eat.param("Mid2Gain", -18.0, 18.0, 0.0, unit="dB"),
        "Mid2Q": eat.param("Mid2Q", 0.1, 10.0, 1.0, unit="Q"),
        "HighFreq": eat.param("HighFreq", 2000.0, 20000.0, 10000.0, unit="Hz"),
        "HighGain": eat.param("HighGain", -18.0, 18.0, 0.0, unit="dB"),
    }
...
def gui():
    return {
        "panel": {
            "title": "PARAMETRIC EQ",
            "subtitle": "5-Band Precision Mastering Equalizer",
            "background": "dark",
            "accent": "#00E5FF",
            "layout": [
                {
                    "type": "row",
                    "children": [
                        {"type": "knob", "param": "HPFFreq", "label": "HPF", "unit": "Hz"},
                        {"type": "knob", "param": "LowFreq", "label": "LOW FREQ", "unit": "Hz"},
                        {"type": "knob", "param": "LowGain", "label": "LOW GAIN", "unit": "dB"},
                        {"type": "knob", "param": "Mid1Freq", "label": "MID1 FREQ", "unit": "Hz"},
                        {"type": "knob", "param": "Mid1Gain", "label": "MID1 GAIN", "unit": "dB"},
                        {"type": "knob", "param": "Mid1Q", "label": "MID1 Q"}
                    ]
                },
                {
                    "type": "row",
                    "children": [
                        {"type": "knob", "param": "Mid2Freq", "label": "MID2 FREQ", "unit": "Hz"},
                        {"type": "knob", "param": "Mid2Gain", "label": "MID2 GAIN", "unit": "dB"},
                        {"type": "knob", "param": "Mid2Q", "label": "MID2 Q"},
                        {"type": "knob", "param": "HighFreq", "label": "HIGH FREQ", "unit": "Hz"},
                        {"type": "knob", "param": "HighGain", "label": "HIGH GAIN", "unit": "dB"}
                    ]
                }
            ]
        }
...
def graph():
    audio_in = eat.node.input()
    hpf = eat.node.svf(audio_in, cutoff="HPFFreq", type="highpass")
    low_shelf = eat.node.svf(hpf, cutoff="LowFreq", gain="LowGain", type="lowshelf")
    mid1 = eat.node.svf(low_shelf, cutoff="Mid1Freq", reso="Mid1Q", gain="Mid1Gain", type="peaking")
    mid2 = eat.node.svf(mid1, cutoff="Mid2Freq", reso="Mid2Q", gain="Mid2Gain", type="peaking")
    high_shelf = eat.node.svf(mid2, cutoff="HighFreq", gain="HighGain", type="highshelf")
    return high_shelf
...
''',
    ...
    EatScriptDef(
      ...
      category: EatScriptCategory.audioFx,
      description: 'Studio VCA bus compressor with auto-makeup gain, punchy attack, and dynamic glue.',
      tags: ['compressor', 'bus', 'glue', 'vca', 'punchy', 'mastering', 'studio'],
      code: r'''
# @id: punchy_bus_comp
# @name: Punchy Bus Compressor
# @category: audioFx
...
def init():
    eat.use_engine("compressor")
    return {
        "Threshold": eat.param("Threshold", -40.0, 0.0, -14.0, unit="dB"),
        "Ratio": eat.param("Ratio", 1.0, 20.0, 4.0, unit=""),
        "Attack": eat.param("Attack", 0.5, 100.0, 10.0, unit="ms"),
        "Release": eat.param("Release", 10.0, 1000.0, 100.0, unit="ms"),
        "Makeup": eat.param("Makeup", 0.0, 24.0, 2.5, unit="dB"),
        "Mix": eat.param("Mix", 0.0, 1.0, 1.0, unit=""),
    }
...
def graph():
    in_sig = eat.node.input()
    crushed = eat.node.bitcrush(in_sig, bits="BRRBits", downsample=2.0)
    filtered = eat.node.svf(crushed, cutoff=8000.0, reso=0.707, type="lowpass")
    saturated = eat.node.saturate(filtered, drive="Drive")
    out = eat.node.mix(in_sig, saturated, mix="Mix")
    return out
...
''',
    ...
    EatScriptDef(
      id: 'stereo_chorus',
      name: 'Stereo Studio Chorus',
      category: EatScriptCategory.audioFx,
      description: 'Dual quadrature LFO stereo chorus and flanger with adjustable rate, depth, feedback, and mix.',
      tags: ['chorus', 'modulation', 'flanger', 'spatial', 'width', 'stereo'],
      code: r'''
# @id: stereo_chorus
# @name: Stereo Studio Chorus
# @category: audioFx
...
def init():
    return {
        "Rate": eat.param("Rate", 0.05, 8.0, 0.8, unit="Hz"),
        "Depth": eat.param("Depth", 0.0, 1.0, 0.65, unit=""),
        "Feedback": eat.param("Feedback", 0.0, 0.85, 0.2, unit=""),
        "Mix": eat.param("Mix", 0.0, 1.0, 0.5, unit=""),
    }
...
def gui():
    return {
        "panel": {
            "title": "STEREO CHORUS",
            "subtitle": "Dual Quadrature LFO Modulation",
            "background": "dark",
            "accent": "#9C27B0",
            "layout": [
                {
                    "type": "row",
                    "children": [
                        {"type": "knob", "param": "Rate", "label": "RATE", "unit": "Hz"},
                        {"type": "knob", "param": "Depth", "label": "DEPTH"},
...
def graph():
    audio_in = eat.node.input()
    chorus = eat.node.chorus(audio_in, rate="Rate", depth="Depth", feedback="Feedback", mix="Mix")
    return chorus
...
def graph():
    in_sig = eat.node.input()
    sat = eat.node.saturate(in_sig=in_sig, drive="Pre")
    return sat
...
def graph():
    strum = eat.node.pluck(spread=8.0, bite="PickAttack")
    strings = eat.node.waveguide(in_sig=strum, damping="StringDamping", feedback=0.995)
    body = eat.node.acoustic_body(in_sig=strings, profile=0.5, gain="BodyWood")
    return body
...
# --- Modular Synthesis Graph Topology ---
def graph():
    osc1 = eat.node.osc(wave="square", freq=540.0)
    osc2 = eat.node.osc(wave="square", freq=800.0)
    osc_mix = eat.node.mix(osc1, osc2)
    decay_env = eat.node.decay(decay="Decay")
    vca = eat.node.gain(osc_mix, gain_cv=decay_env)
    bpf = eat.node.svf(vca, cutoff="Tune", reso=2.2, type="bandpass")
    out = eat.node.saturate(bpf, drive=1.2)
    return out
''',
    ),
    EatScriptDef(
...
# --- Modular Synthesis Graph Topology ---
def graph():
    cluster = eat.node.metallic_cluster(tune="Tune")
    decay_env = eat.node.decay(decay="Decay")
    vca = eat.node.gain(cluster, gain_cv=decay_env)
    bpf = eat.node.svf(vca, cutoff="Cutoff", reso=3.2, type="bandpass")
    out = eat.node.saturate(bpf, drive=1.1)
    return out
''',
    ),
    EatScriptDef(
...
# --- Modular Synthesis Graph Topology ---
def graph():
    pitch_sweep = eat.node.pitch_sweep(start="StartFreq", end="Tune", decay="PitchDecay")
    sine = eat.node.osc(wave="sine", pitch_cv=pitch_sweep)
    amp_env = eat.node.decay(decay="Decay")
    body = eat.node.gain(sine, gain_cv=amp_env)
    tone_filtered = eat.node.svf(body, cutoff="Tone", type="lowpass")
    noise = eat.node.noise()
    click_env = eat.node.decay(decay=0.004)
    click = eat.node.gain(noise, gain_cv=click_env, gain="Click")
    mix = eat.node.mix(tone_filtered, click)
    out = eat.node.saturate(mix, drive="Overdrive")
    return out
''',
    ),
    EatScriptDef(
...
# --- Modular Synthesis Graph Topology ---
def graph():
    body1 = eat.node.osc(wave="sine", freq=180.0)
    body2 = eat.node.osc(wave="sine", freq=330.0)
    body_mix = eat.node.mix(body1, body2)
    body_env = eat.node.decay(decay="ToneDecay")
    body_vca = eat.node.gain(body_mix, gain_cv=body_env)
    noise = eat.node.noise()
    noise_hp = eat.node.svf(noise, cutoff=2000.0, type="highpass")
    wire_env = eat.node.decay(decay="Decay")
    wire_vca = eat.node.gain(noise_hp, gain_cv=wire_env, gain="Snappy")
    mix = eat.node.mix(body_vca, wire_vca)
    out = eat.node.saturate(mix, drive=1.15)
    return out
''',
    ),
    EatScriptDef(
...
def graph():
    ...
    sine = eat.node.osc(wave="sine", pitch_cv=pitch_sweep)
    decay_env = eat.node.decay(decay="Decay")
    vca = eat.node.gain(sine, gain_cv=decay_env)
    shell = eat.node.svf(vca, cutoff=120.0, reso=2.5, gain=3.0, type="peaking")
    out = eat.node.saturate(shell, drive=1.1)
    return out
''',
    ),
    EatScriptDef(
...
def graph():
    noise = eat.node.noise()
    bpf = eat.node.svf(noise, cutoff=1200.0, reso=2.0, type="bandpass")
    burst = eat.node.multi_burst(bursts=4, spread=0.011, decay="Decay")
    vca = eat.node.gain(bpf, gain_cv=burst)
    out = eat.node.saturate(vca, drive=1.2)
    return out
''',
    ),
    EatScriptDef(
...
# --- Modular Synthesis Graph Topology ---
def graph():
    hh = eat.node.tr909_sample(sample="closed_hihat", tune="Tune", decay="Decay")
    hpf = eat.node.svf(hh, cutoff=4000.0, type="highpass")
    return hpf
''',
    ),
    EatScriptDef(
...
# --- Modular Synthesis Graph Topology ---
def graph():
    hh = eat.node.tr909_sample(sample="opened_hihat", tune="Tune", decay="Decay")
    hpf = eat.node.svf(hh, cutoff=3500.0, type="highpass")
    return hpf
''',
    ),
    EatScriptDef(
...
# --- Modular Synthesis Graph Topology ---
def graph():
    rim = eat.node.tr909_sample(sample="rim", tune="Tune", decay="Decay")
    out = eat.node.saturate(rim, drive=1.1)
    return out
''',
    ),
    EatScriptDef(
...
# --- Modular Synthesis Graph Topology ---
def graph():
    snare = eat.node.tr909_snare(tune="Tune", snappy="Snappy", tone="ToneDecay")
    out = eat.node.saturate(snare, drive=1.1)
    return out
''',
    ),
    EatScriptDef(
      ...
      tags: ['pad', 'ambient', 'synth', 'shimmer', 'atmosphere', 'lush'],
      code: r'''
# @id: astral_shimmer_pad
# @name: Astral Shimmer Pad
# @category: instrument
...
def init():
    ...
    return {
        "Cutoff": eat.param("Cutoff", 100.0, 16000.0, 2400.0, unit="Hz"),
        "Resonance": eat.param("Resonance", 0.1, 5.0, 0.55, unit=""),
        "Detune": eat.param("Detune", 1.0, 35.0, 14.0, unit="cents"),
        "Warmth": eat.param("Warmth", 0.0, 1.0, 0.65, unit=""),
        "Attack": eat.param("Attack", 0.05, 4.0, 0.85, unit="s"),
        "Decay": eat.param("Decay", 0.1, 6.0, 1.5, unit="s"),
        "Sustain": eat.param("Sustain", 0.0, 1.0, 0.85, unit=""),
        "Release": eat.param("Release", 0.1, 8.0, 2.5, unit="s"),
    }
...
''',
    ...
    EatScriptDef(
      id: 'c64_sid_drum_kit',
      name: 'C64 SID Drum Kit',
      category: EatScriptCategory.instrument,
      description: 'Authentic MOS 6581 / 8580 Commodore 64 Chiptune Drum Machine: 23-bit Galois LFSR noise snares, triangle chip kicks, high-frequency noise ticks, toms, and cymbals mapped to General MIDI drum notes (35-81).',
      code: r'''
# @id: c64_sid_drum_kit
# @name: C64 SID Drum Kit
# @category: instrument
...
def init():
    return {
        ...
        "KickPunch": eat.param("KickPunch", 0.0, 1.0, 0.75, step=0.0),
        "SnareSnap": eat.param("SnareSnap", 0.0, 1.0, 0.65, step=0.0),
        "NoiseMetal": eat.param("NoiseMetal", 0.0, 1.0, 0.50, step=0.0),
        "ChipModel": eat.param("ChipModel", 0.0, 1.0, 0.0, step=0.0, allow_variance=False),
        "Overdrive": eat.param("Overdrive", 1.0, 3.0, 1.35, step=0.0),
    }
...
def gui():
    return {
        "panel": {
            "title": "C64 SID DRUM KIT",
            "subtitle": "MOS 6581 / 8580 Chiptune Percussion Machine",
            ...
            "layout": [
                {
                    "type": "group",
                    "label": "SID CHIPTUNE PERCUSSION CONTROLS",
                    "accent": "#6C5EB5",
                    "children": [
                        {
                            "type": "row",
...
# --- Synthesizer Voice DSP Process Hook ---
def process(time, freq, note, params):
    # Evaluated by SIDDrumKitEngine in EatDspSynthesizer
    return 0.0
''',
    ),
    EatScriptDef(
...
# --- Modular DSP Synthesis Graph ---
def graph():
    strike = eat.node.hammer(hardness="HammerHardness", click=1.0)
    strings = eat.node.waveguide(in_sig=strike, damping="Brightness", feedback=0.996)
    body = eat.node.acoustic_body(in_sig=strings, profile=0.8, gain="LidOpen")
    return body
...
def init():
    return {
        "waveform": eat.param("waveform", 0.0, 3.0, 1.0, unit=""),
        "pulse_width": eat.param("pulse_width", 0.1, 0.9, 0.45, unit=""),
        "sub_osc": eat.param("sub_osc", 0.0, 1.0, 0.35, unit=""),
        "cutoff": eat.param("cutoff", 50.0, 14000.0, 2800.0, unit="Hz"),
        "resonance": eat.param("resonance", 0.1, 8.0, 2.2, unit=""),
        "env_mod": eat.param("env_mod", -1.0, 1.0, 0.45, unit=""),
        "drive": eat.param("drive", 0.0, 2.0, 0.35, unit=""),
        "attack": eat.param("attack", 0.001, 1.0, 0.008, unit="s"),
        "decay": eat.param("decay", 0.01, 2.0, 0.25, unit="s"),
        "sustain": eat.param("sustain", 0.0, 1.0, 0.65, unit=""),
        "release": eat.param("release", 0.01, 3.0, 0.20, unit="s"),
    }
...
def init():
    return {
        ...
        "GlideCurve": eat.param("GlideCurve", 0.0, 2.0, 0.0, step=1.0, allow_variance=False),
    }
...
''',
    ...
    EatScriptDef(
      ...
      name: 'SNES Sfxr',
      category: EatScriptCategory.instrument,
      description: '16-Bit retro arcade and console sound effect synthesizer with procedural presets, random seeds, and SNES echo DSP.',
      code: r'''
# @name: SNES Sfxr
# @category: instrument
# @description: 16-Bit retro arcade and console sound effect synthesizer with procedural presets, random seeds, and SNES echo DSP.
...
# --- Modular DSP Synthesis Graph ---
def graph():
    strike = eat.node.hammer(hardness="FeltDamping", click=0.5)
    strings = eat.node.waveguide(in_sig=strike, damping="Warmth", feedback=0.993)
    body = eat.node.acoustic_body(in_sig=strings, profile=0.5, gain=0.6)
    return body
...
# --- Modular Synthesis Graph Topology ---
def graph():
    tom = eat.node.melodic_tom(decay="TomDecay", coupling="HeadCoupling", pitch_bend="PitchBend", stick="StickCrack")
    filtered = eat.node.svf(tom, cutoff="Tone", type="lowpass")
    out = eat.node.saturate(filtered, drive=1.1)
    return out
...
# --- Modular DSP Synthesis Graph ---
def graph():
    vco1 = eat.node.osc(wave="saw")
    vco2 = eat.node.osc(wave="saw", detune=4.0)
    sub = eat.node.sub(octave=-1, level=0.6)
    sum = eat.node.mix(in_a=vco1, in_b=vco2)
    vcf = eat.node.moog(in_sig=sum, cutoff="Cutoff", reso="Resonance")
    env = eat.node.adsr(attack=0.01, decay="Decay", sustain=0.4, release=0.2)
...
''',
    ...
    EatScriptDef(
      ...
      category: EatScriptCategory.instrument,
      description: 'Versatile polyphonic & monophonic analog synthesizer with selectable waveforms (Saw, Sine, Square, Triangle, Pulse, Noise), sub-oscillator, resonant state-variable filter (LP/BP/HP), full ADSR, and overdrive.',
      code: r'''
# @id: poly_lead
# @name: Poly Lead Synth
# @category: instrument
...
def init():
    return {
        "Waveform": eat.param("Waveform", 0.0, 5.0, 0.0, step=1.0, options=["SAW", "SINE", "SQR", "TRI", "PULSE", "NOISE"]),
        "PulseWidth": eat.param("PulseWidth", 0.05, 0.95, 0.50, step=0.01),
        "Detune": eat.param("Detune", 0.0, 50.0, 3.0, step=0.1, unit="ct"),
        "SubLevel": eat.param("SubLevel", 0.0, 1.0, 0.0, step=0.01),
        "Cutoff": eat.param("Cutoff", 20.0, 18000.0, 4500.0, step=0.0, unit="Hz"),
        "Resonance": eat.param("Resonance", 0.1, 10.0, 2.5, step=0.0),
        "FilterEnv": eat.param("FilterEnv", -2.0, 2.0, 0.0, step=0.01),
        "FilterMode": eat.param("FilterMode", 0.0, 2.0, 0.0, step=1.0, options=["LP", "BP", "HP"]),
        "Attack": eat.param("Attack", 0.001, 1.0, 0.01, step=0.0, unit="s"),
        "Decay": eat.param("Decay", 0.01, 3.0, 0.20, step=0.0, unit="s"),
        "Sustain": eat.param("Sustain", 0.0, 1.0, 0.75, step=0.01),
        "Release": eat.param("Release", 0.01, 3.0, 0.35, step=0.0, unit="s"),
        "Drive": eat.param("Drive", 0.0, 5.0, 0.0, step=0.01),
    }
...
def gui():
    return {
        "panel": {
            "title": "POLY SYNTH",
            "subtitle": "Multi-Waveform Analog Synthesis & Resonant SVF",
            "accent": "#00FFE0",
            "background": "dark",
            "layout": [
                {
...
# --- Modular DSP Graph Topology ---
def graph():
    vco = eat.node.osc(wave="saw", detune=3.0)
    sub = eat.node.sub(octave=-1, level=0.40)
...
# --- Modular Synthesis Graph Topology ---
def graph():
    cymbal = eat.node.reverse_cymbal(duration="SwellDuration", curve="CrescendoCurve", shimmer="ShimmerAir", choke="ChokeSnap")
    filtered = eat.node.svf(cymbal, cutoff="Tone", type="lowpass")
    return filtered
...
''',
    ...
    EatScriptDef(
      ...
      name: 'SNES Synth',
      category: EatScriptCategory.instrument,
      description: 'Authentic 16-bit polyphonic S-DSP console synthesizer with selectable waveforms, ADSR envelope, vibrato, and hardware echo DSP.',
      code: r'''
# @name: SNES Synth
# @category: instrument
# @description: Authentic 16-bit polyphonic S-DSP console synthesizer with selectable waveforms, ADSR envelope, vibrato, and hardware echo DSP.
...
# --- Modular DSP Synthesis Graph ---
def graph():
    friction = eat.node.bow(pressure="BowPressure", speed=1.0)
    strings = eat.node.waveguide(in_sig=friction, damping=0.22, feedback=0.994)
    body = eat.node.acoustic_body(in_sig=strings, profile=0.25, gain="BodyWood")
    return body
...
# --- Modular Physical Modeling Graph Topology ---
def graph():
    mallet = eat.node.hammer(hardness="MalletSoftness", click=0.8)
    pan_resonator = eat.node.modal_bank(mallet, structure="steelpan")
    body = eat.node.acoustic_body(pan_resonator, profile=0.3, gain="BowlSympathy")
    presence = eat.node.svf(body, cutoff=6000.0, gain="AirPresence", type="highshelf")
    return presence
...
''',
    ...
    EatScriptDef(
      ...
      name: 'Pure Sub Bass',
      category: EatScriptCategory.instrument,
      description: 'Ultra-clean, low-overhead sine and sub-octave bass synthesizer designed for pure sub-bass, 808-style basslines, and punchy acoustic-adjacent low-end anchors.',
      code: r'''
# @id: sub_bass_synth
# @name: Pure Sub Bass
# @category: instrument
...
def init():
    return {
        "Waveform": eat.param("Waveform", 0.0, 5.0, 1.0, step=1.0, options=["SAW", "SINE", "SQR", "TRI", "PULSE", "NOISE"]),
        "SubLevel": eat.param("SubLevel", 0.0, 1.0, 0.60, step=0.01),
        "PulseWidth": eat.param("PulseWidth", 0.05, 0.95, 0.50, step=0.01),
        "Cutoff": eat.param("Cutoff", 20.0, 18000.0, 650.0, step=0.0, unit="Hz"),
        "Resonance": eat.param("Resonance", 0.1, 10.0, 1.0, step=0.0),
        "FilterEnv": eat.param("FilterEnv", -2.0, 2.0, 0.20, step=0.01),
        "FilterMode": eat.param("FilterMode", 0.0, 2.0, 0.0, step=1.0, options=["LP", "BP", "HP"]),
        "Attack": eat.param("Attack", 0.001, 0.5, 0.005, step=0.0, unit="s"),
        "Decay": eat.param("Decay", 0.01, 3.0, 0.60, step=0.0, unit="s"),
        "Sustain": eat.param("Sustain", 0.0, 1.0, 0.80, step=0.01),
        "Release": eat.param("Release", 0.01, 2.0, 0.25, step=0.0, unit="s"),
        "Drive": eat.param("Drive", 0.0, 5.0, 0.15, step=0.01),
        "Detune": eat.param("Detune", 0.0, 50.0, 0.0, step=0.1, unit="ct"),
    }
...
def gui():
    return {
        "panel": {
            "title": "PURE SUB BASS",
            "subtitle": "Zero-Distortion Low Frequency Anchor & 808 Sub",
            "accent": "#00E5FF",
            "background": "matte_metal",
            "layout": [
                {
...
# --- Modular DSP Graph Topology ---
def graph():
    vco = eat.node.osc(wave="sine", detune=0.0)
    sub = eat.node.sub(octave=-1, level=0.60)
    vcf = eat.node.svf(in_sig=vco, cutoff="Cutoff", reso="Resonance")
    env = eat.node.adsr(attack="Attack", decay="Decay", sustain="Sustain", release="Release")
    sat = eat.node.saturate(in_sig=vcf, drive="Drive")
    return sat * env
...
# --- Modular Physical Modeling Graph Topology ---
def graph():
    bachi = eat.node.hammer(hardness="BachiImpact", click=0.9)
    membrane = eat.node.modal_bank(bachi, structure="drum")
    body = eat.node.acoustic_body(membrane, profile=0.8, gain="BarrelBoom")
    sub = eat.node.svf(body, cutoff=100.0, gain=3.0, type="lowshelf")
    return sub
...
# --- Modular DSP Synthesis Graph ---
def graph():
    strike = eat.node.hammer(hardness="MalletHardness", click=1.0)
    bars = eat.node.modal_bank(in_sig=strike, structure="vibraphone")
    tube = eat.node.acoustic_body(in_sig=bars, profile=0.6, gain="MotorTremolo")
    return tube
...
''',
    ...
    EatScriptDef(
      id: 'action_ensemble_arranger',
      name: 'Procedural Ensemble & Dynamic Arranger',
      category: EatScriptCategory.projectAction,
      description: 'Composes multi-track dynamic arrangements across variable-N ensembles (duets, 3-voice trackers, 5-track evolving battle themes, ambient dungeon soundscapes) with instrument dropouts, energy curves, and universal functional roles.',
      tags: ['procgen', 'ensemble', 'arranger', 'rpg', 'orchestration', 'waltz', 'chiptune'],
      code: r'''
# @id: action_ensemble_arranger
# @name: Procedural Ensemble & Dynamic Arranger
...
def init():
    return {
        "Template": eat.param("Template", 0, 5, 0, options=[
        ...
        "Swing": eat.param("Swing", 0.0, 1.0, 0.15, step=0.05),
        "Humanize": eat.param("Humanize", 0.0, 1.0, 0.20, step=0.05),
    }
...
def run(project, params):
    return {
        ...
        "humanize": params.get("Humanize", 0.20),
    }
''',
    ),
    EatScriptDef(
...
''',
    ...
    EatScriptDef(
      id: 'action_procedural_acid_303',
      name: 'Procedural Acid 303 Pattern Generator',
      category: EatScriptCategory.projectAction,
      description: 'Procedurally composes authentic TB-303 acid patterns with 6 classic subgenres, realistic 16th staccato gate timing, 60ms constant-rate portamento slides, syncopated off-beat accents, and optional companion 909 beats.',
      tags: ['procgen', 'acid', '303', 'tb303', 'bass', 'techno', 'macro'],
      code: r'''
# @id: action_procedural_acid_303
# @name: Procedural Acid 303 Pattern Generator
# @category: projectAction
...
def init():
    return {
        "Style": eat.param("Style", 0, 5, 0, options=[
        ...
        "Root": eat.param("Root", 0, 12, 0, options=[
            "Auto (From Song Key)",
            "C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "B"
        ]),
        "Scale": eat.param("Scale", 0, 5, 0, options=[
            "Minor Pentatonic",
            "Phrygian (Dark)",
            "Dorian (Groovy)",
            "Natural Minor",
        ...
        "Bars": eat.param("Bars", 1, 8, 2, step=1),
        "Density": eat.param("Density", 0.30, 1.0, 0.75, step=0.05),
        "TieProbability": eat.param("TieProbability", 0.0, 0.80, 0.30, step=0.05),
        "SlideProbability": eat.param("SlideProbability", 0.0, 1.0, 0.40, step=0.05),
        "AccentProbability": eat.param("AccentProbability", 0.0, 1.0, 0.35, step=0.05),
        "OctaveJumpRange": eat.param("OctaveJumpRange", 0, 2, 1, options=[
            "1 Octave (Tight Bass)",
            "2 Octaves (Classic 303 Leaps)",
            "3 Octaves (Wide Goa Arps)"
        ]),
        "Waveform": eat.param("Waveform", 0, 2, 0, options=[
            "Sawtooth (Buzzy / Classic)",
            "Square (Hollow / Aggressive)",
            "Keep Existing / Random"
        ]),
        "Companion909": eat.param("Companion909", 0, 1, 0, options=[
            "None (Acid Bassline Only)",
            "4-on-Floor 909 Groove (Kick + Open Hat + Clap)"
        ]),
        "Seed": eat.param("Seed", 1, 999999, 303, step=1),
    }
...
def run(project, params):
    return {
        ...
        "tie_probability": params.get("TieProbability", 0.30),
        "slide_probability": params.get("SlideProbability", 0.40),
        "accent_probability": params.get("AccentProbability", 0.35),
        "octave_jump_range": params.get("OctaveJumpRange", 1),
        "waveform": params.get("Waveform", 0),
        "companion_909": params.get("Companion909", 0),
        "seed": params.get("Seed", 303),
    }
''',
    ),
    EatScriptDef(
...
def init():
    return {
        "Style": eat.param("Style", 0, 7, 0, options=["Lo-Fi Hip Hop", "Synthwave / Retrowave", "Deep House", "Cyberpunk Acid", "Neo-Soul / R&B", "Pop / Funk Anthem", "SNES 16-Bit Adventure", "C64 SID 8-Bit Chiptune"]),
