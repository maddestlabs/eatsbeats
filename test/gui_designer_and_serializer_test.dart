...
void main() {
  ...
  group('Eatscript GUI Serializer & Roundtrip Tests', () {
    test('EatGuiSerializer serializes complete panel with knobs, sliders, nixies, and canvases', () {
      const panel = EatScriptGuiPanelDef(
        title: 'MY ACID SYNTH',
        ...
        children: [
          EatScriptGuiNode(
            ...
            children: [
              EatScriptGuiNode(type: EatScriptGuiNodeType.knob, param: 'Cutoff', label: 'CUTOFF', unit: 'Hz', size: 56, knobStyle: KnobStyle.chrome),
              EatScriptGuiNode(type: EatScriptGuiNodeType.knob, param: 'Resonance', label: 'RESO', size: 48),
              EatScriptGuiNode(type: EatScriptGuiNodeType.nixie, param: 'Accent', label: 'ACCENT', unit: '%', width: 100),
            ],
          ),
          EatScriptGuiNode(
            type: EatScriptGuiNodeType.row,
            children: [
              EatScriptGuiNode(type: EatScriptGuiNodeType.slider, param: 'Drive', label: 'OVERDRIVE', orientation: 'horizontal', width: 440, sliderStyle: SliderStyle.console),
            ],
          ),
          EatScriptGuiNode(
            type: EatScriptGuiNodeType.row,
            children: [
              EatScriptGuiNode(type: EatScriptGuiNodeType.spaceVisualizer, height: 140),
            ],
          ),
        ],
      );

      final eatCode = EatGuiSerializer.serialize(panel: panel, instrumentName: 'MyAcidSynth');
      ...
      // Roundtrip test with EatGuiParser for EatScript
      final parsedEat = EatGuiParser.parseFromCode(eatCode);
      ...
      // Also verify Eatscript serialization
      final eatScriptCode = EatGuiSerializer.serializeToEatScript(panel: panel, instrumentName: 'MyAcidSynth');
      expect(eatScriptCode, contains('def gui():'));
      expect(eatScriptCode, contains('"title": "MY ACID SYNTH"'));
      final parsedEatScript = EatGuiParser.parseFromCode(eatScriptCode);
      expect(parsedEatScript, isNotNull);
      expect(parsedEatScript!.title, equals('MY ACID SYNTH'));
    });

    test('EatGuiSerializer handles nested columns, custom hex background, track accent, and label/value visibility', () {
      const advancedPanel = EatScriptGuiPanelDef(
        ...
        children: [
          EatScriptGuiNode(
            type: EatScriptGuiNodeType.row,
            children: [
              EatScriptGuiNode(type: EatScriptGuiNodeType.knob, param: 'Cutoff', label: 'CUTOFF', showLabel: false, showValue: false),
              EatScriptGuiNode(
                type: EatScriptGuiNodeType.column,
                children: [
                  EatScriptGuiNode(type: EatScriptGuiNodeType.slider, param: 'Attack', label: 'ATTACK', orientation: 'horizontal', width: 120),
                  EatScriptGuiNode(type: EatScriptGuiNodeType.slider, param: 'Decay', label: 'DECAY', orientation: 'horizontal', width: 120),
                  EatScriptGuiNode(type: EatScriptGuiNodeType.switchToggle, param: 'Mute', orientation: 'vertical', showLabel: false),
                ],
              ),
              EatScriptGuiNode(type: EatScriptGuiNodeType.nixie, param: 'Freq', showLabel: false, unit: 'Hz'),
              EatScriptGuiNode(type: EatScriptGuiNodeType.oscilloscope, width: 280, height: 120),
              EatScriptGuiNode(type: EatScriptGuiNodeType.spectrum, width: 280, height: 120),
            ],
          ),
        ],
      );

      final eatCode = EatGuiSerializer.serialize(panel: advancedPanel, instrumentName: 'ModularStack');
      expect(eatCode, contains('def gui():'));
      ...
      final parsed = EatGuiParser.parseFromCode(eatCode);
      ...
      expect(parsed.children[0].type, equals(EatScriptGuiNodeType.row));
      expect(parsed.children[0].children[0].showLabel, isFalse);
      expect(parsed.children[0].children[0].showValue, isFalse);
      expect(parsed.children[0].children[1].type, equals(EatScriptGuiNodeType.column));
      ...
      expect(parsed.children[0].children[3].type, equals(EatScriptGuiNodeType.oscilloscope));
      expect(parsed.children[0].children[4].type, equals(EatScriptGuiNodeType.spectrum));

      // Also verify Eatscript roundtrip
      final eatScriptCode = EatGuiSerializer.serializeToEatScript(panel: advancedPanel, instrumentName: 'ModularStack');
      expect(eatScriptCode, contains('def gui():'));
      final parsedEatScript = EatGuiParser.parseFromCode(eatScriptCode);
      expect(parsedEatScript, isNotNull);
      expect(parsedEatScript!.backgroundStyle, equals(PanelBackgroundStyle.custom));
    });

    test('EatGuiSerializer and EatGuiParser roundtrip backgroundSvgStrokeWidth, row opacity, borderWidth, and borderColor', () {
      const panel = EatScriptGuiPanelDef(
        ...
        children: [
          EatScriptGuiNode(
            ...
            borderWidth: 0.0,
            children: [
              EatScriptGuiNode(type: EatScriptGuiNodeType.knob, param: 'Downpour', label: 'DOWNPOUR'),
              EatScriptGuiNode(type: EatScriptGuiNodeType.knob, param: 'Impact', label: 'DROP IMPACT'),
            ],
          ),
          EatScriptGuiNode(
            ...
            children: [
              EatScriptGuiNode(
                type: EatScriptGuiNodeType.row,
                children: [
                  EatScriptGuiNode(type: EatScriptGuiNodeType.knob, param: 'Cavity', label: 'CAVITY RESO'),
                ],
              ),
            ],
          ),
      ...
      final serialized = EatGuiSerializer.serialize(panel: panel, instrumentName: 'TestInstrument');
      ...
      final parsed = EatGuiParser.parseFromCode(serialized);
      expect(parsed, isNotNull);
      ...
      // Also verify Eatscript serializer
      final eatSerialized = EatGuiSerializer.serializeToEatScript(panel: panel, instrumentName: 'TestInstrument');
      expect(eatSerialized, contains('"backgroundSvgStrokeWidth": 0.75'));
      expect(eatSerialized, contains('"opacity": 0.0'));
      expect(eatSerialized, contains('"borderWidth": 0.0'));
      expect(eatSerialized, contains('"opacity": 0.25'));
      expect(eatSerialized, contains('"borderWidth": 1.5'));
      expect(eatSerialized, contains('"borderColor": "#00E5FF"'));
      expect(parsed.children[0].borderWidth, closeTo(0.0, 0.01));
      expect(parsed.children[0].children.length, equals(2));
      expect(parsed.children[1].type, equals(EatScriptGuiNodeType.group));
    ...
    test('EatGuiSerializer and EatGuiParser roundtrip backgroundGradient and backgroundSvgLayers', () {
      const panel = EatScriptGuiPanelDef(
        ...
        children: [
          EatScriptGuiNode(
            type: EatScriptGuiNodeType.row,
            children: [
              EatScriptGuiNode(type: EatScriptGuiNodeType.nixie, param: 'Algorithm', label: 'ALGORITHM'),
              EatScriptGuiNode(type: EatScriptGuiNodeType.nixie, param: 'Feedback', label: 'FEEDBACK'),
            ],
          ),
        ],
      );

      final serialized = EatGuiSerializer.serializeToEatScript(panel: panel, instrumentName: 'Opl3Hardware');
    ...
    test('EatGuiSerializer.ensureGuiBlock injects def gui(): block into scripts without one', () {
      const rawCode = '''# @name: Simple Tone
def init():
    eat.param("Pitch", 440.0, 20.0, 2000.0, "Hz")
    eat.param("Volume", 0.8, 0.0, 1.0, "")

def process():
    return 0.0
''';

      final codeWithGui = EatGuiSerializer.ensureGuiBlock(rawCode, instrumentName: 'SimpleTone');
      expect(codeWithGui, contains('def gui():'));
      expect(codeWithGui, contains('"title": "SIMPLETONE"'));
      expect(codeWithGui, contains('"param": "Pitch"'));
      expect(codeWithGui, contains('"param": "Volume"'));
    });
  });
...
''';
      ...
      expect(currentScript, contains('def gui():'));
    });
...
''',
    ...
    test('EatScriptGuiSerializer roundtrips meter, drumPads, dpad, group, segmentedPill, label, and spacer', () {
      const panel = EatScriptGuiPanelDef(
        ...
        children: [
          EatScriptGuiNode(
            type: EatScriptGuiNodeType.row,
            children: [
              EatScriptGuiNode(type: EatScriptGuiNodeType.meter, size: 120),
              EatScriptGuiNode(type: EatScriptGuiNodeType.drumPads),
              EatScriptGuiNode(type: EatScriptGuiNodeType.dpad),
              EatScriptGuiNode(type: EatScriptGuiNodeType.spacer, size: 24),
            ],
          ),
          EatScriptGuiNode(
            type: EatScriptGuiNodeType.group,
            ...
            children: [
              EatScriptGuiNode(type: EatScriptGuiNodeType.label, text: 'MODE SECTION'),
              EatScriptGuiNode(
      ...
      final serialized = EatGuiSerializer.serialize(panel: panel, instrumentName: 'ExtendedRack');
      ...
      expect(parsed!.children.length, equals(2));
      expect(parsed.children[0].type, equals(EatScriptGuiNodeType.row));
      expect(parsed.children[0].children[0].type, equals(EatScriptGuiNodeType.meter));
      expect(parsed.children[0].children[1].type, equals(EatScriptGuiNodeType.drumPads));
      expect(parsed.children[0].children[2].type, equals(EatScriptGuiNodeType.dpad));
      expect(parsed.children[0].children[3].type, equals(EatScriptGuiNodeType.spacer));
      expect(parsed.children[1].type, equals(EatScriptGuiNodeType.group));
      expect(parsed.children[1].children[0].type, equals(EatScriptGuiNodeType.label));
      expect(parsed.children[1].children[1].type, equals(EatScriptGuiNodeType.segmentedPill));
