...
void main() {
  ...
  group('MIDI FX Eatscript Script Presets Tests', () {
    test('All MIDI FX presets in EatScriptLibrary compile cleanly with valid params & GUIs', () {
      final midiPresets = EatScriptLibrary.presets.where((p) => p.isMidiFx).toList();
      expect(midiPresets, isNotEmpty);

      for (final p in midiPresets) {
        final result = EatEngine.compile(p.code);
  ...
  group('DawState Script Targets & MIDI FX Management Tests', () {
    ...
    test('MIDI FX insert contains full Eatscript code and compiles with 0 syntax errors', () {
      final dawState = DawState();
      final track = dawState.activeTrack;
      final arpPreset = EatScriptLibrary.getPresetById('arpeggiator_midi_fx')!;
      ...
      expect(mfx.eatScriptCode, contains('init()'));
      expect(mfx.eatScriptCode, contains('transform_notes'));
      ...
      expect(code, equals(mfx.eatScriptCode));
    ...
    test('Master Bus Audio FX appear in getAllScriptTargets', () {
      ...
      final filterPreset = EatScriptLibrary.getPresetById('lowpass_filter')!;
